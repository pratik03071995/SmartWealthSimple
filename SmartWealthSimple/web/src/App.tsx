...
export default function App() {
  ...
  const activeRoute = useMemo<Route>(() => {
    ...
    if (pathname.startsWith("/earnings"))  return ROUTES[2];
    if (pathname.startsWith("/learn"))     return ROUTES[3];
    if (pathname.startsWith("/community")) return ROUTES[4];
    if (pathname.startsWith("/emerging"))  return ROUTES[5];
    return ROUTES[0];
  }, [pathname]);
