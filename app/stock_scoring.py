...
class StockScorer:
    ...
    def get_financial_data(self, ticker: str) -> Dict:
        ...
        try:
            ...
            # Use get_info() which is more reliable in recent yfinance versions
            try:
                info = stock.get_info()
            except Exception:
                # Fallback to legacy property if needed
    ...
    def score_innovation_rd(self, data: Dict) -> Tuple[float, Dict]:
        ...
        try:
            # Employee count as proxy for talent (yfinance keys vary)
            employees = (
                info.get('fullTimeEmployees') or
                info.get('full_time_employees') or
                info.get('employees') or
                info.get('employeeCount') or
                info.get('numberOfEmployees') or 0
            )
            ...
            try:
                employees_fmt = f"{int(employees):,}" if isinstance(employees, (int, float)) else str(employees)
            except Exception:
                employees_fmt = str(employees)
            details['innovation'] = f"Sector: {sector}, Employees: {employees_fmt}"
