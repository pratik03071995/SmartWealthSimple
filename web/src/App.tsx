...
import Sectors from "./components/Sectors";               // NEW
import StockScoring from "./components/StockScoring";     // NEW
...
const ROUTES: Route[] = [
  ...
  { path: "/sectors",   key: "sectors",   title: "Sectors",   element: <Sectors /> },           // NEW
  { path: "/scoring",   key: "scoring",   title: "Scoring",   element: <StockScoring /> },      // NEW
...
export default function App() {
  ...
  const activeRoute = useMemo<Route>(() => {
    if (pathname.startsWith("/sectors"))   return ROUTES[1];
    if (pathname.startsWith("/scoring"))   return ROUTES[2];
    if (pathname.startsWith("/earnings"))  return ROUTES[3];
    if (pathname.startsWith("/learn"))     return ROUTES[4];
    if (pathname.startsWith("/community")) return ROUTES[5];
    if (pathname.startsWith("/emerging"))  return ROUTES[6];
    return ROUTES[0];
  }, [pathname]);
